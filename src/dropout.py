...
class Layer:
    ...
    def __init__(self, M1, M2):
        ...
        self.tf_W = tf.Variable(dtype=tf.float32,
                                initial_value=tf.random.normal(shape=(M1, M2), mean=0,
                                                               stddev=1 / M1))  # xavier initialization
        self.tf_b = tf.Variable(dtype=tf.float32,
                                initial_value=tf.random.normal(shape=(M2,)))
...
class NeuralNetwork:
    ...
    def fit(self, Xtrain, ytrain, Xval, yval, epoch=100, learning_rate=0.001, batch_size=50):
        """
        train model
        ...
        with tf.Session() as session:
            ...
            for i in range(epoch):
                ...
                for j in range(nBatches + 1):
                    ...
                    if (iteration % 100 == 0 or (i == epoch - 1 and j == nBatches)):
                        print('epoch ' + str(i) + '/ iteration ' + str(iteration))
                        ...
                        trainingError = session.run(tf_cost_testing, feed_dict={tf_X: Xtrain, tf_Y: Ytrain}) / len(
                            Xtrain)
...
def main():
    ...
    s = NeuralNetwork(hiddenLayersSize=[64, 32], pkeep=[0.8, 0.5, 0.5]) # by default,  pkeep of input = 0.8
